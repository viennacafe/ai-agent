...
## v3.1 PostsApiController.delete 문법 오류 수정

Sequence 정규화기가 participant 식별자에 포함된 점(`.`), 괄호, 슬래시 등의 문자를 안전한 underscore alias로 변환합니다.

예를 들어 모델이 다음처럼 생성하더라도:
...
저장 전 자동으로 다음처럼 변환됩니다.

```mermaid
participant PostsApiController_delete as "PostsApiController"
Client->>PostsApiController_delete: DELETE /posts/{id}
```